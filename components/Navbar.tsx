...
export default function Navbar() {
    ...
    const navLinks = (
        <>
            <Link href="/explore" className="text-[#6b6b6b] hover:text-[#c9a84c] transition-colors" onClick={() => setMobileOpen(false)}>
                Explore
            </Link>
            <Link href="/explore?q=" className="text-[#6b6b6b] hover:text-[#c9a84c] transition-colors flex items-center gap-1.5" onClick={() => setMobileOpen(false)}>
    ...
    return (
        <>
            <nav className={`flex items-center justify-between px-6 lg:px-12 py-5 bg-[#0a0a0a] text-[#f0ece4] sticky top-0 z-50 transition-colors duration-300 ${scrolled ? 'border-b border-[#c9a84c]' : 'border-b border-transparent'}`}>
                ...
                <div className="hidden md:flex items-center space-x-8 text-sm font-medium tracking-wide">
                    ...
                    <SignedOut>
                        <div className="flex items-center space-x-6">
                            ...
                            <span className="text-[#c9a84c] border border-[#c9a84c] px-6 py-2.5 rounded-[16px] hover:bg-[#c9a84c] hover:text-[#0a0a0a] transition-colors cursor-pointer">
                                <SignUpButton />
                            </span>
                        </div>
                    </SignedOut>
                ...
                {/* Mobile hamburger */}
                <button
                    className="md:hidden p-2 text-[#f0ece4] hover:text-[#c9a84c] transition-colors"
            ...
            {/* Mobile overlay */}
            {mobileOpen && (
                <div className="fixed inset-0 z-40 bg-[#0a0a0a]/95 backdrop-blur-lg pt-20 px-8 flex flex-col space-y-6 text-lg font-medium md:hidden animate-in fade-in">
                    {navLinks}
                    <SignedOut>
                        <span className="cursor-pointer text-[#6b6b6b] hover:text-[#f0ece4] transition-colors">
                            <SignInButton />
                        </span>
                        <span className="text-[#c9a84c] border border-[#c9a84c] px-6 py-3 rounded-[16px] hover:bg-[#c9a84c] hover:text-[#0a0a0a] transition-colors cursor-pointer text-center block">
                            <SignUpButton />
                        </span>
                    </SignedOut>
                    <SignedIn>
                        <Link href="/new" className="text-[#6b6b6b] hover:text-[#c9a84c] transition-colors" onClick={() => setMobileOpen(false)}>
                            Write
                        </Link>
                        <Link href="/dashboard" className="text-[#6b6b6b] hover:text-[#c9a84c] transition-colors" onClick={() => setMobileOpen(false)}>
                            Dashboard
                        </Link>
                        <div className="pt-4">
                            <UserButton />
