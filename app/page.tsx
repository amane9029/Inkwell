  ...
  return (
    <div className="bg-[#0d0d0d] min-h-screen font-sans selection:bg-[#c9a84c]/20 text-[#f0ece4] flex flex-col">
      ...
      <main className="max-w-6xl mx-auto w-full px-6 py-8 flex-1" id="explore">
        {!posts && !error && <div className="py-20 text-center text-[#6b6b6b] text-lg font-light animate-pulse">Loading amazing stories...</div>}
        {error && <div className="py-20 text-center text-red-400 font-medium">Failed to load posts</div>}
        {posts?.length === 0 && (
          <div className="py-24 text-center border border-[#2a2a2a] rounded-[20px] bg-[#141414]">
            ...
            <Link href="/new" className="bg-[#c9a84c] text-[#0a0a0a] px-8 py-3 rounded font-bold hover:bg-[#e8c96a] transition-all inline-block">
              Start writing
            </Link>
          </div>
        )}
          ...
          <div className="space-y-24">
                ...
                <div className="flex flex-col md:flex-row bg-[#1a1a1a] rounded-[20px] relative overflow-hidden transition-transform duration-500 hover:-translate-y-1 hover:shadow-2xl hover:shadow-[#000]/50 border-l-4 border-l-[#c9a84c]">
                  ...
                  <div className="w-full md:w-[55%] p-10 md:p-14 flex flex-col justify-center relative z-10">
                    <div className="flex items-center space-x-3 text-xs text-[#6b6b6b] mb-6 font-medium uppercase tracking-wider">
                      <span>{posts[0].profiles?.name || 'Unknown Writer'}</span>
                      <span>·</span>
                    ...
                    <div className="text-[#6b6b6b] text-lg font-light leading-relaxed line-clamp-3 mb-8" dangerouslySetInnerHTML={{ __html: posts[0].content || '' }}></div>
                  ...
                  <div className="w-full md:w-[45%] h-64 md:h-auto bg-[#111] relative overflow-hidden rounded-r-[20px]">
                    ...
                    {posts[0].cover_image_url ? (
                      ...
                      <img src={posts[0].cover_image_url} alt={posts[0].title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-[1.5s] ease-out opacity-80 group-hover:opacity-100" />
                    ) : (
                    ...
                    <article key={post.id} className="group flex flex-col bg-[#141414] border border-[#2a2a2a] rounded-[16px] overflow-hidden hover:border-[#c9a84c] hover:-translate-y-1 hover:shadow-xl hover:shadow-[#000]/60 transition-all duration-300 h-full">
                      <Link href={`/p/${post.id}`} className="block relative h-[200px] bg-[#0a0a0a] overflow-hidden border-b border-[#2a2a2a]">
                        {post.cover_image_url ? (
                          // eslint-disable-next-line @next/next/no-img-element
                          <img src={post.cover_image_url} alt={post.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-[1s] ease-out opacity-70 group-hover:opacity-100" />
                        ) : (
                      ...
                      <div className="p-8 flex flex-col flex-1">
                        <Link href={`/p/${post.id}`} className="block mb-6 flex-1">
                          ...
                          <div className="text-[#6b6b6b] text-sm leading-relaxed font-light line-clamp-2" dangerouslySetInnerHTML={{ __html: post.content || '' }}></div>
                        </Link>
            ...
            <section className="py-2">
              ...
              <div className="flex overflow-x-auto space-x-3 pb-4 scrollbar-hide -mx-6 px-6 sm:mx-0 sm:px-0">
                {['Technology', 'Design', 'Startups', 'Culture', 'Programming', 'Literature', 'Poetry', 'Business', 'Art'].map(topic => (
                  <button key={topic} className="flex-shrink-0 px-6 py-2.5 rounded-full border border-[#2a2a2a] bg-[#1a1a1a] text-sm font-medium text-[#f0ece4] hover:bg-transparent hover:border-[#c9a84c] hover:text-[#c9a84c] transition-colors">
                    {topic}
                  </button>
                ))}
              </div>
      ...
      <div className="w-full px-6 mb-24 mt-12 mx-auto" style={{ maxWidth: '1400px' }}>
        <section className="bg-[#111111] rounded-[32px] py-24 px-8 lg:px-16 overflow-hidden max-w-6xl mx-auto border border-[#2a2a2a]/30">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div className="bg-[#1a1a1a] rounded-[24px] p-10 hover:border-t-[#c9a84c] border-t-[3px] border-t-transparent transition-all duration-300 group shadow-lg shadow-[#000]/20">
              <div className="w-10 h-10 rounded-lg bg-[#c9a84c]/10 text-[#c9a84c] flex items-center justify-center mb-6 border border-[#c9a84c]/20">
      ...
      <footer className="bg-[#0a0a0a] border-t border-[#c9a84c]/20 pt-16 pb-8">
        <div className="max-w-6xl mx-auto px-6 flex flex-col items-center">
          ...
          <div className="flex space-x-8 mb-12 text-sm text-[#6b6b6b] uppercase tracking-widest font-medium">
            <Link href="#" className="hover:text-[#c9a84c] transition-colors">About</Link>
            <Link href="/new" className="hover:text-[#c9a84c] transition-colors">Write</Link>
            <Link href="#" className="hover:text-[#c9a84c] transition-colors">Privacy</Link>
            <Link href="#" className="hover:text-[#c9a84c] transition-colors">Terms</Link>
          </div>
